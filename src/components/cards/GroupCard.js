      ...
      return (
        ...
        >
          <div className="tile-content">
            {name} <img src={PlayIconSVG} alt="Play" />{" "}
          </div>
        </div>
      );
    }}
...
export default function GroupCard({ item, index }) {
  return (
    <Draggable draggableId={`group-${index}`} index={index} key={item.name}>
      {(provided, snapshot) => (
        <div
          className="group__card drag_card_container"
          ref={provided.innerRef}
          {...provided.draggableProps}
          {...provided.dragHandleProps}
          style={getItemStyle(
          ...
          <div className="card__body">
            {item.childs.length > 0 && (
              <div className="card__body_header">
                <span>
                  Running{" "}
                  {item.childs[0].type === "stream"
                    ? "Streams"
                    : item.childs[0].type === "simulation"
                    ? "Simulations"
                    : "Stream Processors"}
                </span>
                <div className="right">
