    ...
    return (
      <div className="Valuecard">
        ...
        <div className="cardFooter">
          <a href={`/projects/${item.project && item.project.id}/streamprocessors/${item.id}/edit/`} className='edit'>
             {/*EDIT - Needs to go to - /projects/2/streamprocessors/3/edit/ -->*/}
            <span className="helper">Edit</span>
          </a>
          <a href={`/projects/${item.project && item.project.id}/streamprocessors/${item.id}/duplicate/`} className='duplicate'>
             {/*DUPLICATE -  Needs to go to - /projects/2/streamprocessors/3/duplicate/-->*/}
            <span className="helper">Duplicate</span>
          </a>
          <a href={`/projects/${item.project && item.project && item.project.id}/streamprocessors/${item.id}/run/`} className='deploy'>
             {/*DEPLOY - Needs to go to - /projects/2/streamprocessors/3/run/ -->*/}
            <span className="helper">Deploy</span>
          </a>
          <a href={`/projects/${item.project && item.project.id}/streamprocessors/${item.id}/stop/`} className='pause'>
           {/*STOP - Needs to go to - /projects/2/streamprocessors/3/stop/ -->*/}
            <span className="helper">Stop</span>
          </a>
          <a href={`/projects/${item.project && item.project.id}/streamprocessors/${item.id}/monitor/`} className='monitor'>
           {/*MONITOR - Needs to go to - /projects/2/streamprocessors/3/monitor/ -->*/}
            <span className="helper">Monitor</span>
          </a>
          <a href={`/projects/${item.project && item.project.id}/streamprocessors/${item.id}/delete/`} className='delete'>
