...
export default function CardBoardLayout(props) {
  ...
  const multiSelect = (entities, selectedRowIds, newRowId) => {
  ...
  const onDragStart = (start) => {
    ...
    if (id.indexOf("group-") > -1) setIsCombine(false);
    else setIsCombine(true);
  ...
  const multiSelectTo = (newRowId) => {
    const updated = multiSelect(items, selectedRowIds, newRowId);
  ...
  return (
    ...
    >
      ...
      >
        {(provided, snapshot) => (
          <div
            className="card_board_container"
