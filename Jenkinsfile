...
//git env vars
env.git_url = 'git@github.com:benjaminwootton1981/timeflow-admin-gui-react.git'
        ...
        stage('BUILD'){
            stages {
                stage('Start Build'){
                    steps {
                        script {
                            notifySlack("Timeflow Admin GUI React job is started to build! Build logs from jenkins server $jenkins_server_url/job/$JOB_NAME/$BUILD_NUMBER/console", notification_channel)
                        }
                    }
                }
                ...
                stage('End Build'){
                    steps {
                        script {
                            notifySlack("Timeflow Admin GUI job is completed BUILD PHASE! Build logs from jenkins server $jenkins_server_url/job/$JOB_NAME/$BUILD_NUMBER/console", notification_channel)
                        }
                    }
                }
            }
